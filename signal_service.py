...
class SignalService(TWSConnector):
    ...
    def request_historical_data(self, symbol: str, days: int = 90) -> int:
        ...
        """
        contract = Contract()
        # Internationale Ticker-Erkennung
        if symbol.endswith('.DE'):
            contract.symbol = symbol.replace('.DE', '')
            contract.secType = "STK"
            contract.exchange = "IBIS"
            contract.currency = "EUR"
        elif symbol.endswith('.PA'):
            contract.symbol = symbol.replace('.PA', '')
            contract.secType = "STK"
            contract.exchange = "Euronext Paris"
            contract.currency = "EUR"
        else:
...
def main():
    ...
    try:
        ...
        # Test Pushover - DEAKTIVIERT (wird vom Options-Scanner übernommen)
